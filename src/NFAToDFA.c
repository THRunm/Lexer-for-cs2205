...
// 暴力建构DFA
void dfsHash(bool *father, char last, bool *src, struct finite_automata_exinf *d, struct star_edges *c,
             struct finite_automata_exinf *dst, int *types) {
...
struct finite_automata *NFA2DFA(int n, struct finite_automata **nfa, int *dst, int *dst_number, int **ans_dst,
    ...
    struct finite_automata *anss = malloc(sizeof(struct finite_automata));
    anss->n = ans.n;
    anss->m = ans.m;
    anss->src = ans.src;
    anss->dst = ans.dst;
